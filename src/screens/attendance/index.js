...
import { fetchAttendanceData } from "../../redux/reducer/attendanceReducer";
...
const AttendanceScreen = () => {
  ...
  const attendanceData = useSelector((state) => state.attendanceDataFetch.data);
  const loading = useSelector((state) => state.attendanceDataFetch.loading);
  const error = useSelector((state) => state.attendanceDataFetch.error);
  ...
  const markedDates = {};
  // Mark present dates as green
  data &&
    data[0]?.present?.forEach((item) => {
      const date = item.date.substring(0, 10); // Extract the date part (YYYY-MM-DD)
      markedDates[date] = { selected: true, selectedColor: "green" };
    });

  // // Mark absent dates as red
  data &&
    data[0]?.absent?.forEach((item) => {
      const date = item.date.substring(0, 10); // Extract the date part (YYYY-MM-DD)
