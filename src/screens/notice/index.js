                ...
                renderItem={({ item }) => {
                    return (
                        <DropShadow style={globalstyles.dropShadow}>
                            <TouchableOpacity style={{ height: Height(80), width: Width(350), backgroundColor: colors.whiteColor, alignSelf: 'center', marginTop: Height(30), borderRadius: Width(5), padding: Width(20), justifyContent: 'center' }} onPress={() => navigation.navigate('NoticeDetails', { data: item })}>
                                <Text style={{ fontSize: Height(20), fontFamily: fonts.ARCHIVO_MEDIUM, color: colors.textColor }}>{item.Notice_title}</Text>
                            </TouchableOpacity>
                        </DropShadow>
                    )
                }}
