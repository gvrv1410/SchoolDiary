...
const HomeWorkScreen = () => {
    ...
    const getDaysInMonth = (month, year) => {
        // Note: Month is zero-based (0-January, 1-February, etc.)
        ...
        <View style={globalstyles.container}>
            <View>
                ...
                <DropShadow style={styles.firstViewShadow}>
                    <TouchableOpacity style={styles.headerBtn} onPress={handleToggleDropdown}>
                        <Text style={styles.dropdownItemText}>{selectedMonth || 'None'} {getCurrentYear()}</Text>
                    </TouchableOpacity>
                </DropShadow>
            </View>
                    ...
                    <View style={styles.dateList}>
                        {selectedMonthDates.map((date, index) => (
                            <DropShadow style={globalstyles.dropShadow} key={index}>
                                <TouchableOpacity key={index} style={styles.dateItem} onPress={() => navigation.navigate('HomeWorkList', { date: date, data: selectedMonth, year: getCurrentYear() })}>
                                    <LinearGradient colors={[colors.linearFirst, colors.linearSecond]} style={styles.dateRoundView}>
...
const styles = StyleSheet.create({
    ...
    headerBtn: {
        ...
        height: Height(400), width: Width(200),
        backgroundColor: colors.whiteColor, borderRadius: Width(5)
    }, dropdownItemText: {
        fontSize: Height(20),
        ...
        fontSize: Height(18), fontFamily: fonts.ARCHIVO_MEDIUM, color: colors.blackColor, marginLeft: Width(20)
    }, dateText: {
        fontSize: Height(30), fontFamily: fonts.ARCHIVO_SEMIBOLD, color: colors.textColor
    },
    dateRoundView: {
        height: Height(50), width: Height(50), borderRadius: Height(50) / 2, justifyContent: 'center', alignItems: 'center'
    }, scrollView: {
        zIndex: -1, marginTop: Height(50)
    }
})